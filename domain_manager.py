...
RDAP_OVERRIDES = {"ch": "https://rdap.nic.ch/domain/", "li": "https://rdap.nic.li/domain/"}
sessions = {}
...
def fetch_rdap(domain_name):
    ...
    url = RDAP_OVERRIDES.get(tld, "https://rdap.org/domain/") + urllib.parse.quote(domain_name, safe="")
    ...
    with urllib.request.urlopen(request, timeout=15) as response:
        body = response.read()
        if "json" not in str(response.headers.get("Content-Type", "")).lower() and body.lstrip().startswith(b"<"):
            raise ValueError("RDAP 返回了网页而不是注册数据")
...
def fetch_whois_text(host, query):
    if not re.fullmatch(r"[a-z0-9.-]+", host, re.I):
        raise ValueError("WHOIS 服务器地址无效")
    chunks = []
    size = 0
    with socket.create_connection((host, 43), timeout=12) as connection:
        connection.settimeout(12)
        connection.sendall((query + "\r\n").encode("idna"))
        while size < 1024 * 1024:
            chunk = connection.recv(min(16384, 1024 * 1024 - size))
            if not chunk:
                break
            chunks.append(chunk)
            size += len(chunk)
...
def whois_referral(tld):
    if tld not in whois_referrals:
        text = fetch_whois_text("whois.iana.org", tld)
        match = re.search(r"^whois:\s*([^\s]+)", text, re.I | re.M)
        if not match:
            raise ValueError("该域名后缀没有公开 WHOIS 服务")
        whois_referrals[tld] = match.group(1).strip().lower()
    return whois_referrals[tld]
...
def normalize_whois(text):
    def values(*keys):
        names = "|".join(re.escape(key) for key in keys)
        return [value.strip() for value in re.findall(rf"^\s*(?:{names})\s*:\s*(.+)$", text, re.I | re.M)]

    created = values("Creation Date", "Created", "Registered", "registered")
    expires = values("Registry Expiry Date", "Expiration Date", "Expiry Date", "expire", "paid-till", "renewal date")
    updated = values("Updated Date", "Last Updated", "Modified", "changed")
    ...
    problem_statuses = {"redemptionperiod", "pendingdelete", "serverhold", "clienthold", "inactive"}
    return {
        "created_at": whois_date(created[0]) if created else "",
        "expires_at": whois_date(expires[0]) if expires else "",
        "updated_at": whois_date(updated[0]) if updated else "",
...
def fetch_registration(domain_name):
    labels = domain_name.lower().strip(".").split(".")
    candidates = [".".join(labels[index:]) for index in range(len(labels) - 1)]
    for candidate in candidates:
        try:
            record = fetch_rdap(candidate)
            record.update({"source": "RDAP", "queried_name": candidate, "is_parent": candidate != domain_name})
            return record
        except (OSError, UnicodeError, ValueError):
            pass
    for candidate in candidates:
        try:
            text = fetch_whois_text(whois_referral(candidate.rsplit(".", 1)[-1]), candidate)
            record = normalize_whois(text)
            record.update({"source": "WHOIS", "queried_name": candidate, "is_parent": candidate != domain_name})
            return record
        except (OSError, UnicodeError, ValueError):
            pass
    raise ValueError("该域名没有可用的公开 RDAP 或 WHOIS 档案")
...
class DomainHandler(BaseHTTPRequestHandler):
    ...
    def domain_whois(self, domain_id):
        ...
        try:
            whois = fetch_registration(domain["name"])
        except (OSError, UnicodeError, ValueError):
            if cached:
                return self.send_json(200, {"whois": cached, "cached": True, "stale": True, "warning": "WHOIS 查询暂时失败，当前显示上次查询结果"})
            return self.send_json(502, {"error": "没有查到公开注册档案；该后缀可能不支持查询，或注册局暂时不可用"})
        with store_lock:
            store = load_store(self.data_dir)
...
.toolbar{margin-bottom:24px}.toolbar h1{letter-spacing:0}.page-kicker{display:flex;align-items:center;gap:8px;color:var(--primary);font-size:12px;font-weight:850;text-transform:uppercase;margin-bottom:7px}.page-kicker:before{content:'';width:18px;height:2px;background:var(--primary)}.overview{grid-template-columns:minmax(0,1.15fr) minmax(320px,.85fr);gap:0;margin:0 0 30px;border-block:1px solid var(--border);background:transparent}.asset-band,.rank-section{padding:24px 0;background:transparent}.asset-band{padding-right:28px}.rank-section{padding-left:28px;border-left:1px solid var(--border)}.metric-title,.rank-title{margin-bottom:16px}.metric-title h2,.rank-title h2{font-size:15px;color:var(--muted)}.metric-value{font-size:38px}.metric-grid{display:flex;gap:0;margin-top:22px}.mini-metric{flex:1;padding:0 18px;border:0;border-left:1px solid var(--border);border-radius:0;background:transparent}.mini-metric:first-child{padding-left:0;border-left:0}.mini-metric b{font-size:21px;margin-top:4px}.rank-list{gap:0}.rank-list.scrollable{max-height:190px;overflow-y:auto;padding-right:7px;scrollbar-color:var(--border) transparent;scrollbar-width:thin}.rank-item{padding:10px 0;border:0;border-top:1px solid var(--border);border-radius:0;background:transparent;transition:padding .18s,background .18s}.rank-item:hover{padding-left:8px;padding-right:8px;background:var(--input)}.rank-item:first-child{border-top:0}.rank-index{border-radius:7px}.rank-name-line,.domain-line{display:flex;align-items:center;gap:8px;min-width:0}.rank-name-line .rank-name{min-width:0}.section-bar{display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin:0 0 14px}.section-heading{display:grid;gap:3px}.section-bar h2{margin:0;font-size:18px}.section-count{color:var(--muted);font-size:13px}.asset-tools{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}.asset-tools input{width:210px}.asset-tools select{width:auto;min-width:126px}.grid{gap:16px}.card{min-height:210px;animation:card-in .36s cubic-bezier(.2,.8,.2,1) both;animation-delay:calc(var(--i,0)*45ms)}@keyframes card-in{from{opacity:0;transform:translateY(12px) scale(.985)}to{opacity:1;transform:none}}.card:hover{transform:translateY(-4px)}.card.og,.card.premium{border-color:#d8b95e88}.card.og{box-shadow:inset 0 1px 0 #fff8cf55,0 22px 55px #9a762924}.card.og:before{height:5px;background:#e0b94f}.card.premium:before{background:#b99a45}.card.og:hover,.card.premium:hover{border-color:#e3c66c;box-shadow:inset 0 1px 0 #fff8cf66,0 26px 64px #9a762938}.asset-badge{flex:0 0 auto;padding:4px 7px;border:1px solid #d8b95e88;border-radius:999px;background:#f2d47724;color:#a47b16;font-size:10px;font-weight:900;line-height:1;text-transform:uppercase}.asset-badge.og{background:#e0b94f;color:#211b0a;border-color:#e0b94f;box-shadow:0 4px 13px #b5892440}html[data-theme=dark] .asset-badge{color:#f1d57d}@media(prefers-color-scheme:dark){html[data-theme=auto] .asset-badge{color:#f1d57d}}.card .status{transition:transform .18s}.card:hover .status{transform:translateY(-1px)}.card>*:not(.registrar-mark){position:relative;z-index:1}.card .meta .row:last-child{padding-right:56px}.tag-line{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.auto-badge{border-radius:999px;padding:5px 9px;background:var(--success-soft);color:var(--success);font-size:12px;font-weight:800}.registrar-mark{position:absolute;right:12px;bottom:10px;z-index:0;width:48px;height:48px;padding:8px;display:grid;place-items:center;border:1px solid var(--glass-border);border-radius:15px;background:color-mix(in srgb,var(--glass-strong) 82%,transparent);color:var(--muted);font-size:18px;font-weight:900;opacity:.82;box-shadow:inset 0 1px 0 var(--glass-highlight),0 6px 18px #00000012;pointer-events:none}.registrar-mark svg,.registrar-mark img{display:block;width:100%;height:100%;object-fit:contain}.registrar-mark.registrar-generic{padding:0}
.rank-name-line,.domain-line{flex-wrap:wrap}.asset-badge.age{border-color:var(--border);background:var(--input);color:var(--muted);text-transform:none;box-shadow:none}
...
select option{background:#fff;color:#172033}html[data-theme=dark] select option{background:#242a33;color:#f2f4f7}@media(prefers-color-scheme:dark){html[data-theme=auto] select option{background:#242a33;color:#f2f4f7}}
...
@media(max-width:820px){.section-bar{align-items:stretch}.asset-tools{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));width:100%}.asset-tools input{grid-column:1/-1;width:100%}.asset-tools select{width:100%;min-width:0}}
...
<script>
const $=s=>document.querySelector(s);let domains=[],current=null,settings=null,settingsTab='rules',displayCurrency=localStorage.getItem('dm_currency')||'CNY',assetSort=localStorage.getItem('dm_asset_sort')||'expiry_asc',assetSearch='',statusFilter='all',tierFilter='all',renewFilter='all',exchangeRates={USD:1,CNY:7.2,EUR:.86,GBP:.75,HKD:7.8,JPY:145},exchangeRatesLoaded=false;
...
async function api(path,opts={}){const r=await fetch(path,{headers:{'Content-Type':'application/json'},...opts});if(r.status===401){showLogin();throw new Error('请先登录')}const t=await r.text();let data={};if(t)try{data=JSON.parse(t)}catch(e){throw new Error(r.ok?'服务器返回了无法识别的数据':'服务器或反向代理暂时不可用，请稍后重试')}if(!r.ok)throw new Error(data.error||'请求失败');return data}
...
function annualCost(d){const price=renewalInfo(d);return price?convertedAmount(price.amount/price.years,price.currency,'USD'):null}
// ponytail: curated common words; add manual overrides if automatic classification needs full coverage.
const OG_WORDS=new Set('apple art bank best blue book cao car cash cloud code coin data day design dream earth echo film fire food game gold green home host hotel john king life light link love mail map market media moon music news nova open pay photo pixel play queen red shop sky smart space sport star store sun tech time travel type video web work world'.split(' '));
function assetTier(d){const label=String(d.name||'').split('.')[0].toLowerCase();if(!/^[a-z0-9]+$/.test(label))return null;if(label.length<=2||OG_WORDS.has(label))return['og','OG'];if(label.length<=4)return['premium','Premium'];return null}
function domainAge(d,now=new Date()){const match=/^(\d{4})-(\d{2})-(\d{2})/.exec(d.whois?.created_at||'');if(!match)return 0;const year=Number(match[1]),month=Number(match[2]),day=Number(match[3]);let age=now.getFullYear()-year;if(now.getMonth()+1<month||(now.getMonth()+1===month&&now.getDate()<day))age--;return age>=6?age:0}
function tierBadge(d){const tier=assetTier(d),age=domainAge(d);return `${tier?`<span class="asset-badge ${tier[0]}">${tier[1]}</span>`:''}${age?`<span class="asset-badge age">${age}年老域名</span>`:''}`}
...
REGISTRAR_LOGOS.quyu=`<img alt="" src="data:image/png;base64,">`;
function registrarMark(registrar){const raw=String(registrar||'').trim(),name=raw.toLowerCase();const known=[['quyu.net','quyu'],['趣域网','quyu'],['趣域','quyu'],['quyu','quyu'],['cloudflare','cloudflare'],['spaceship','spaceship'],['regery','regery'],['namecheap','namecheap'],['godaddy','godaddy'],['阿里','aliyun'],['aliyun','aliyun'],['alibaba','aliyun'],['腾讯','tencent'],['tencent','tencent']];if(name==='cf')return{kind:'cloudflare',html:REGISTRAR_LOGOS.cloudflare};for(const [key,kind] of known)if(name.includes(key))return{kind,html:REGISTRAR_LOGOS[kind]};return{kind:'generic',html:`<span>${escapeHtml(raw?[...raw][0].toUpperCase():'◎')}</span>`}}
function filteredDomains(){const query=assetSearch.trim().toLowerCase(),list=domains.filter(d=>{const n=daysLeft(effectiveExpiry(d)),state=statusInfo(d)[0],tier=assetTier(d)?.[0]||'standard';if(query&&!`${d.name} ${effectiveRegistrar(d)||''}`.toLowerCase().includes(query))return false;if(statusFilter==='due'&&!(n!==''&&n>=0&&n<=90))return false;if(statusFilter==='expired'&&!(n!==''&&n<0))return false;if(statusFilter==='healthy'&&state!=='ok')return false;if(tierFilter!=='all'&&tierFilter!==tier)return false;if(renewFilter==='auto'&&!d.auto_renew)return false;if(renewFilter==='manual'&&d.auto_renew)return false;return true});const direction=assetSort.endsWith('_desc')?-1:1,key=assetSort.replace(/_(asc|desc)$/,'');return list.sort((a,b)=>{let av,bv;if(key==='name')return a.name.localeCompare(b.name)*direction;if(key==='length'){av=a.name.split('.')[0].length;bv=b.name.split('.')[0].length}else if(key==='price'){av=annualCost(a);bv=annualCost(b)}else if(key==='value'){av=remainingValue(a);bv=remainingValue(b)}else{av=daysLeft(effectiveExpiry(a));bv=daysLeft(effectiveExpiry(b))}const missingA=av===null||av===''||!Number.isFinite(av),missingB=bv===null||bv===''||!Number.isFinite(bv);if(missingA!==missingB)return missingA?1:-1;return missingA?a.name.localeCompare(b.name):(av-bv)*direction||a.name.localeCompare(b.name)})}
function renderAssetBar(){let bar=document.getElementById('sectionBar');if(!bar){bar=document.createElement('div');bar.id='sectionBar';bar.className='section-bar';cards.before(bar)}bar.innerHTML=`<div class="section-heading"><h2>域名资产</h2><span class="section-count" id="sectionCount"></span></div><div class="asset-tools"><input id="assetSearchInput" type="search" value="${escapeAttr(assetSearch)}" placeholder="搜索域名或注册商" aria-label="搜索域名或注册商"><select id="statusFilterSelect" aria-label="按状态筛选"><option value="all">全部状态</option><option value="healthy">状态正常</option><option value="due">90 天内到期</option><option value="expired">已过期</option></select><select id="tierFilterSelect" aria-label="按类型筛选"><option value="all">全部类型</option><option value="og">OG 域名</option><option value="premium">Premium 域名</option><option value="standard">普通域名</option></select><select id="renewFilterSelect" aria-label="按续费方式筛选"><option value="all">全部续费方式</option><option value="auto">自动续费</option><option value="manual">手动续费</option></select><select id="assetSortSelect" aria-label="域名排序"><option value="expiry_asc">到期：近到远</option><option value="expiry_desc">到期：远到近</option><option value="name_asc">名称：A-Z</option><option value="length_asc">长度：短到长</option><option value="length_desc">长度：长到短</option><option value="price_asc">年费：低到高</option><option value="price_desc">年费：高到低</option><option value="value_desc">价值：高到低</option></select></div>`;$('#statusFilterSelect').value=statusFilter;$('#tierFilterSelect').value=tierFilter;$('#renewFilterSelect').value=renewFilter;$('#assetSortSelect').value=assetSort;$('#assetSearchInput').oninput=e=>{assetSearch=e.target.value;renderCards()};$('#statusFilterSelect').onchange=e=>{statusFilter=e.target.value;renderCards()};$('#tierFilterSelect').onchange=e=>{tierFilter=e.target.value;renderCards()};$('#renewFilterSelect').onchange=e=>{renewFilter=e.target.value;renderCards()};$('#assetSortSelect').onchange=e=>{assetSort=e.target.value;localStorage.setItem('dm_asset_sort',assetSort);renderCards()}}
function renderCards(){const visible=filteredDomains();$('#sectionCount').textContent=`${visible.length} / ${domains.length} 个域名`;cards.innerHTML='';if(!domains.length){cards.innerHTML='<div class="empty">还没有域名，点右上角新增。</div>';return}if(!visible.length){cards.innerHTML='<div class="empty">没有符合当前筛选条件的域名。</div>';return}visible.forEach((d,i)=>{const [state,label]=statusInfo(d),registrar=effectiveRegistrar(d),expiry=effectiveExpiry(d),tier=assetTier(d);const remaining=remainingValue(d),mark=registrarMark(registrar);const b=document.createElement('button');b.className=`card ${state==='danger'?'danger':state==='warn'?'warn':''} ${tier?.[0]||''}`;b.style.setProperty('--i',i);b.innerHTML=`<div class="registrar-mark registrar-${mark.kind}" aria-hidden="true">${mark.html}</div><div class="card-top"><div class="domain-line"><span class="domain">${escapeHtml(d.name)}</span>${tierBadge(d)}</div><span class="status ${state==='danger'?'danger':state==='warn'?'warn':''}">${label}</span></div><div class="tag-line"><span class="tag">${escapeHtml(registrar||'未填写注册商')}</span>${d.auto_renew?'<span class="auto-badge">自动续费</span>':''}</div><div class="meta"><div class="row"><span class="label">到期时间</span><strong class="value">${escapeHtml(expiryText(expiry))}</strong></div><div class="row"><span class="label">续费价格</span><strong class="value" title="原价：${escapeAttr(d.renewal_price||'未填写')}">${escapeHtml(convertedPrice(d))}</strong></div><div class="row"><span class="label">剩余价值</span><strong class="value">${escapeHtml(remaining===null?'无法估算':money(remaining))}</strong></div></div>`;b.onclick=()=>openDetail(d.id);cards.appendChild(b)})}
