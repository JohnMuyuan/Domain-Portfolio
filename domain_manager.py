...
function assetTier(d){const label=String(d.name||'').split('.')[0];if(!/^[a-z0-9]+$/i.test(label))return null;if(label.length<=3)return['og','OG'];if(label.length<=5)return['premium','Premium'];return null}
function tierBadge(d){const tier=assetTier(d);return tier?`<span class="asset-badge ${tier[0]}">${tier[1]}</span>`:''}
