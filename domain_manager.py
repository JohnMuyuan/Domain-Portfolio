...
async function loadDomains(){showApp();showList();overview.innerHTML='<div class="skeleton skeleton-band"></div><div class="skeleton skeleton-band"></div>';cards.innerHTML='<div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div><div class="skeleton skeleton-card"></div>';try{const data=await api('/api/domains');domains=data.domains;renderList()}catch(e){toast(e.message,'error')}}
...
function renderList(){showList(false);renderOverview();let bar=document.getElementById('sectionBar');if(!bar){bar=document.createElement('div');bar.id='sectionBar';bar.className='section-bar';cards.before(bar)}bar.innerHTML=`<h2>域名资产</h2><span class="section-count">${domains.length} 个域名</span>`;cards.innerHTML='';if(!domains.length){cards.innerHTML='<div class="empty">还没有域名，点右上角新增。</div>';return}domains.forEach((d,i)=>{const [state,label]=statusInfo(d.expires_at);const remaining=remainingValue(d);const b=document.createElement('button');b.className=`card ${state==='danger'?'danger':state==='warn'?'warn':''}`;b.style.setProperty('--i',i);b.innerHTML=`<div class="card-top"><span class="domain">${escapeHtml(d.name)}</span><span class="status ${state==='danger'?'danger':state==='warn'?'warn':''}">${label}</span></div><span class="tag">${escapeHtml(d.registrar||'未填写注册商')}</span><div class="meta"><div class="row"><span class="label">到期时间</span><strong class="value">${escapeHtml(expiryText(d.expires_at))}</strong></div><div class="row"><span class="label">续费价格</span><strong class="value">${escapeHtml(d.renewal_price||'未填写')}</strong></div><div class="row"><span class="label">剩余价值</span><strong class="value">${escapeHtml(remaining?money(remaining.amount,remaining.symbol):'无法估算')}</strong></div></div>`;b.onclick=()=>openDetail(d.id);cards.appendChild(b)})}
function blank(){return{id:null,name:'',expires_at:'',registrar:'',renewal_price:'',renewal_url:'',notes:''}}
